...
//modified for indexed palette images

#include "xbrz_indexed.h"
#include <cassert>
#include <algorithm>
...
namespace
	...
	uint8_t* byteAdvance(uint8_t* ptr, int bytes)
		{
		return ptr + bytes;
		}

	const uint8_t* byteAdvance(const uint8_t* ptr, int bytes)
		{
		return ptr + bytes;
		}
	...
	void fillBlock(uint8_t* trg, int pitch, uint8_t col, int blockWidth, int blockHeight)
		{
		for(int y = 0; y < blockHeight; y++)
			{
			memset(trg, col, blockWidth);
			trg += pitch;
			}
		}
...
void xbrz::nearestNeighborScale(const uint8_t* src, int srcWidth, int srcHeight, int srcPitch,
								uint8_t* trg, int trgWidth, int trgHeight, int trgPitch,
								SliceType st, int yFirst, int yLast)
	{
	if(srcPitch < srcWidth * static_cast<int>(sizeof(uint8_t)) ||
	   trgPitch < trgWidth * static_cast<int>(sizeof(uint8_t)))
		{
		assert(false);
		return;
		}
	...
	switch(st)
		...
		case NN_SCALE_SLICE_SOURCE:
			...
			for(int y = yFirst; y < yLast; ++y)
				...
				if(blockHeight > 0)
					{
					const uint8_t* srcLine = byteAdvance(src, y * srcPitch);
					uint8_t* trgLine = byteAdvance(trg, yTrg_first * trgPitch);
					int xTrg_first = 0;
		...
		case NN_SCALE_SLICE_TARGET:
			...
			for(int y = yFirst; y < yLast; ++y)
				{
				//uint32_t* trgLine = byteAdvance(trg, );
				const int ySrc = srcHeight * y / trgHeight;
				//const uint32_t* srcLine = byteAdvance(src, );
				for(int x = 0; x < trgWidth; ++x)
					{
					const int xSrc = srcWidth * x / trgWidth;
					trg[y * trgPitch + x] = src[ySrc * srcPitch + xSrc];
					}
				}
			break;
		}
