...
void resourceManager::GetBackdrop(char* filename, bitmap* screenBuf, int isICN) {
	if (isICN) {
		icon* icn = GetIcon(filename);
		icn->DrawToBuffer(0, 0, 0, 0);
		Dispose(icn);
	} else {
		unsigned int bmpHash = MakeId(filename, 1);
		PointToFile(bmpHash);
		ReadWord();
		ReadWord();
		ReadWord();
		ReadBlock(screenBuf->contents, screenBuf->width * screenBuf->height);
	}
}
...
bitmap* resourceManager::GetBitmap(char* filename) {
	bitmap *bmp;
	unsigned int hash;

	hash = MakeId(filename, 1);
	bmp = (bitmap*)Query(hash);
	if(bmp != NULL) {
		++bmp->referenceCount;
	} else {
		bmp = new bitmap(hash);
...
icon* resourceManager::GetIcon(char* filename) {
	return GetIcon(MakeId(filename, 1));
}

icon* resourceManager::GetIcon(unsigned long hash) {
	icon *icn = (icon *)Query(hash);
	if(icn != NULL) {
		++icn->referenceCount;
	} else {
		icn = new icon(hash);
...
font* resourceManager::GetFont(char* filename) {
	unsigned int hash = MakeId(filename, 1);
	font *fnt = (font*)Query(hash);
	if(fnt) {
		++fnt->referenceCount;
	} else {
		fnt = new font(hash);
...
sample* resourceManager::GetSample(const std::string &filename) {
	...
	std::copy(filename.begin(), filename.end(), charname);
	charname[filename.size()] = '\0';
	sample* samp = this->GetSample(charname);
	delete[] charname;
	return samp;
}
...
unsigned long resourceManager::MakeId(char* filename, int allowEvil) {
	...
	if(gbUseEvilInterface && allowEvil) {
		for(int i = 0; i < NUM_GOOD_EVIL_ICONS; i++) {
			if(!stricmp(this->resourceToLoad, cEvilTranslate[i][0]))
