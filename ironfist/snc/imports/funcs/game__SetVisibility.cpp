  ...
  if ( radius < 10 )
  {
    for ( rowa[0] = y - radius; ; ++rowa[0] )
    {
      ...
      for ( rowa[1] = x - radius; radius + x >= rowa[1]; ++rowa[1] )
      {
        ...
        if ( v10 <= v5 - abs(x - rowa[1]) && *(_QWORD *)rowa >= 0i64 && MAP_WIDTH > rowa[1] && rowa[0] < MAP_HEIGHT )
          *(&mapExtra[rowa[1]] + rowa[0] * MAP_WIDTH) |= playerMask;
      }
    }
  }
  else
    ...
    for ( row = 0; ; ++row )
    {
      ...
      for ( col = 0; MAP_WIDTH > col; ++col )
      {
        if ( (signed int)(signed __int64)sqrt((double)((y - row) * (y - row) + (x - col) * (x - col))) < radius )
          *(&mapExtra[col] + row * MAP_WIDTH) |= playerMask;
      }
    }
  }
  return result;
