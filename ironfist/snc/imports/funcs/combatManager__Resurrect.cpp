{
  ...
  if ( this->heroes[this->currentActionSide] && hero::HasArtifact(this->heroes[this->currentActionSide], ARTIFACT_ANKH) )
    spellpower *= 2;
  v17 = combatManager::FindResurrectArmyIndex(thisa, thisa->currentActionSide, spell, hex);
        ...
        if ( *(&thisa->combatGrid[0].corpseOwners[99 * v9] + spriteIdx - v9) == thisa->currentActionSide )
        {
          if ( *(&thisa->combatGrid[0].corpseStackIndices[99 * v9] + spriteIdx - v9) == v17 )
          {
            ...
            if ( !v19 )
            {
              ...
              else if ( !*(&thisa->combatGrid[0].field_33[99 * v9] + spriteIdx - v9) )
              {
                v8 = v9 + 1;
              }
            }
  ...
  if ( creat->quantity - v15 <= 1 )
    sprintf(gText, "%d %s rises from the dead!", creat->quantity - v15, gArmyNames[creat->creatureIdx]);
  else
    sprintf(gText, "%d %s rise from the dead!", creat->quantity - v15, gArmyNamesPlural[creat->creatureIdx]);
