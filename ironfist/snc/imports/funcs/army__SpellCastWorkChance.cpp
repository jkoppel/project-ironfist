{
  ...
  if ( !gpCombatManager->heroes[this->owningSide] )
    goto LABEL_168;
  if ( hero::HasArtifact(gpCombatManager->heroes[this->owningSide], ARTIFACT_HOLY_PENDANT)
  ...
  if ( hero::HasArtifact(gpCombatManager->heroes[thisa->owningSide], ARTIFACT_PENDANT_OF_FREE_WILL)
    ...
    return 0.0;
  if ( hero::HasArtifact(gpCombatManager->heroes[thisa->owningSide], ARTIFACT_PENDANT_OF_LIFE)
    && (spell == SPELL_DEATH_RIPPLE || spell == SPELL_DEATH_WAVE) )
    return 0.0;
  if ( hero::HasArtifact(gpCombatManager->heroes[thisa->owningSide], ARTIFACT_SERENITY_PENDANT)
    && spell == SPELL_BERZERKER )
    return 0.0;
  if ( hero::HasArtifact(gpCombatManager->heroes[thisa->owningSide], ARTIFACT_SEEING_EYE_PENDANT)
    && spell == SPELL_BLIND )
    return 0.0;
  if ( hero::HasArtifact(gpCombatManager->heroes[thisa->owningSide], ARTIFACT_KINETIC_PENDANT)
...
LABEL_168:
    ...
    if ( gpCombatManager->heroes[gpCombatManager->currentActionSide]
      && hero::HasArtifact(gpCombatManager->heroes[gpCombatManager->currentActionSide], ARTIFACT_ANKH) )
...
LABEL_169:
        ...
        || thisa->effectStrengths[14] )
      {
        ...
        if ( hero::HasArtifact(gpCombatManager->heroes[gpCombatManager->currentActionSide], ARTIFACT_GOLD_WATCH) )
          v4 *= 2;
