{
  ...
  v92 = 43;
  v93 = 21;
  if ( !gbNoShowCombat && this->checkedInsub4c3d60 && *(_DWORD *)&combatArmyInfoLevel && !gbInDrawSmallView )
  {
    gbInDrawSmallView = 1;
      ...
      *(_DWORD *)&thisa->_16[4 * a2] = offsetY;
      ...
      if ( v3 )
      {
        ...
        if ( *(_DWORD *)&combatArmyInfoLevel == 2 )
        {
          ...
          sprintf(gText, "%d", *(_BYTE *)(v21 + 198));
          font::DrawBoundedString(smallFont, gText, offsetXa + 8, offsetY + 20, 57, 12, 1, 2);
          sprintf(gText, "%d", *(_BYTE *)(v21 + 199));
          font::DrawBoundedString(smallFont, gText, offsetXa + 8, offsetY + 29, 57, 12, 1, 2);
          sprintf(gText, "%d", *(_WORD *)(v21 + 194));
          font::DrawBoundedString(smallFont, gText, offsetXa + 8, offsetY + 38, 57, 12, 1, 2);
          sprintf(gText, "%d-%d", *(_BYTE *)(v21 + 200), *(_BYTE *)(v21 + 201));
          font::DrawBoundedString(smallFont, gText, offsetXa + 8, offsetY + 47, 57, 12, 1, 2);
          if ( *(_DWORD *)(v21 + 242) <= 0 )
          {
            if ( *(_DWORD *)(v21 + 242) >= 0 )
            {
              icon::DrawToBuffer(thisa->combatScreenIcons[11], offsetXa + 45, offsetY + 56, 5, 0);
            }
            else
            {
              for ( i = 0; -*(_DWORD *)(v21 + 242) > i; ++i )
                icon::DrawToBuffer(thisa->combatScreenIcons[11], offsetXa + 57 - 10 * i, offsetY + 56, 3, 0);
            }
          }
          else
          {
            for ( j = 0; *(_DWORD *)(v21 + 242) > j; ++j )
              icon::DrawToBuffer(thisa->combatScreenIcons[11], offsetXa + 57 - 10 * j, offsetY + 56, 4, 0);
          }
          if ( *(_DWORD *)(v21 + 246) <= 0 )
          {
            if ( *(_DWORD *)(v21 + 246) >= 0 )
            {
              icon::DrawToBuffer(thisa->combatScreenIcons[11], offsetXa + 50, offsetY + 65, 2, 0);
            }
            else
            {
              for ( k = 0; -*(_DWORD *)(v21 + 246) > k; ++k )
                icon::DrawToBuffer(thisa->combatScreenIcons[11], offsetXa + 57 - 10 * k, offsetY + 65, 0, 0);
            }
          }
          else
          {
            for ( l = 0; *(_DWORD *)(v21 + 246) > l; ++l )
              icon::DrawToBuffer(thisa->combatScreenIcons[11], offsetXa + 57 - 10 * l, offsetY + 65, 1, 0);
          }
          if ( *(_BYTE *)(v21 + 208) & 4 )
          {
            sprintf(gText, "%d", *(_BYTE *)(v21 + 202));
        ...
        for ( m = 0; m < v9; ++m )
        {
          ...
          v6 = ((19 - GetIconEntry(thisa->combatScreenIcons[12], spriteIdx)->width) >> 1)
             + (unsigned __int8)*(&v22 + 4 * (3 * v9 - 3) + 2 * m)
             + offsetXa
             + 6;
          v7 = GetIconEntry(thisa->combatScreenIcons[12], spriteIdx);
          icon::DrawToBuffer(thisa->combatScreenIcons[12], v6, ((19 - v7->height) >> 1) + v5, spriteIdx, 0);
        }
      }
      gbLimitToExtent = v94;
      offsetXb = offsetXa - 6;
