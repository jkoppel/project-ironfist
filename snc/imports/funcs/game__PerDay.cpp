{
  ...
  {
    for ( i = 0; i < 7; ++i )
      *(_DWORD *)&gpGame->players[playerIdx]._4[4 * i + 60] = -this->players[playerIdx].resources[i];
  }
  ...
  for ( playerIdxa = 0; playerIdxa < 144; ++playerIdxa )
  {
    if ( this->_8[7 * playerIdxa + 1] != -1 )
    {
      ...
      else if ( this->_8[7 * playerIdxa + 2] )
      {
        if ( v9 != 6 )
          v10 = 1;
      }
    ...
    for ( j = 0; j < 6; ++j )
    {
      if ( this->playerHandicap[playerIdxf] )
      {
        ...
        this->players[playerIdxf].resources[j] -= (signed __int64)((double)(this->players[playerIdxf].resources[j]
                                                                          + *(_DWORD *)&gpGame->players[playerIdxf]._4[4 * j + 60])
                                                                 * v5);
      }
    }
  }
  ...
  {
    result = (char *)gpGame->numPlayers;
    if ( (signed int)result <= playerIdxg )
      break;
    for ( k = 0; k < 7; ++k )
      *(_DWORD *)&gpGame->players[playerIdxg]._4[4 * k + 60] += this->players[playerIdxg].resources[k];
  }
  for ( playerIdxh = 0; playerIdxh < 54; ++playerIdxh )
  {
    thisa = &this->heroes[playerIdxh];
    v3 = thisa->spellpoints;
    v24 = 10 * hero::Stats(thisa, PRIMARY_SKILL_KNOWLEDGE);
  ...
  for ( playerIdxi = 0; playerIdxi < 72; ++playerIdxi )
  {
    ...
    if ( this->castles[playerIdxi].buildingsBuiltFlags & 1 )
    {
      ...
      if ( result != (char *)-1 )
      {
        ...
        v25 = 10 * hero::Stats(v6, PRIMARY_SKILL_KNOWLEDGE);
