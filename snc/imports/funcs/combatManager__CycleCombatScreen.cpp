{
  ...
  for ( j = 0; j < 2; ++j )
  {
    ...
    {
      v13 = &gpCombatManager->creatures[j][i];
      if ( !(gpCombatManager->creatures[j][i].creature.creature_flags & DEAD)
        ...
        && (v13->animationType >= 8 && v13->animationType <= 12
         || v13->animationType == 7 && v13->baseFidgetTime + *(_DWORD *)&v13->gap_129[158] < KBTickCount()) )
      {
        ++v12;
    ...
    if ( thisa->heroIcon[i] )
    {
        ...
        && thisa->heroIconIdxRelated[i] != 11 )
      {
        if ( thisa->heroIconIdxRelated[i] || *(&thisa->field_33A1 + i) || !thisa->field_339D[i] )
        {
          ...
          {
            if ( !thisa->heroIconIdxRelated[i] && thisa->field_33BD[i] + 4500 < KBTickCount() )
            {
              if ( byte_4F4C30[thisa->heroType[i]][0] <= 1 )
                v14[i] = 9;
              else
                v14[i] = Random(0, byte_4F4C30[thisa->heroType[i]][0] - 1) + 9;
              *(&thisa->field_F41F + i) = 1;
            }
          }
          else if ( thisa->playerID[thisa->currentActionSide] != -1
  ...
  if ( v12 || thisa->field_F41F || thisa->field_F423 || thisa->field_F427[0] || thisa->field_F427[1] )
  {
    ...
    for ( k = 0; k < 2; ++k )
    {
      ...
      {
        v13 = &gpCombatManager->creatures[k][i];
        if ( creatureShouldDoSomeAnimation[k][i] )
        {
          if ( v13->animationType == ANIMATION_TYPE_STANDING )
          {
            v5 = (double)Random(1, 100) / 100.0;
            v6 = 0.0;
            v13->field_4A = v13->gap_129[117] - 1;
            for ( l = 0; v13->gap_129[117] - 1 > l; ++l )
            {
              v1 = *(float *)&v13->gap_129[4 * l + 118] + v6;
              v6 = v1;
              if ( v1 > v5 )
              {
                v13->field_4A = l;
            ...
            v13->animationType = v13->field_4A + 8;
            v13->animationFrame = 0;
          }
          else
          {
            ++v13->animationFrame;
            if ( !*(_DWORD *)&v13->gap_129[158] && v13->gap_129[117] == 1 && Random(0, 100) < 8 )
              --v13->animationFrame;
            if ( v13->gap_129[v13->field_4A + 251] <= v13->animationFrame )
            {
              v13->animationType = 7;
              v13->animationFrame = 0;
              v13->baseFidgetTime = KBTickCount();
              if ( *(_DWORD *)&v13->gap_129[158] > 0 )
              {
                v2 = Random(0, *(_DWORD *)&v13->gap_129[158]);
                v13->baseFidgetTime = (signed __int64)((double)v2 * 0.5
                                                     - (double)*(signed int *)&v13->gap_129[158] * 0.25
                                                     + (double)v13->baseFidgetTime);
              }
            }
          }
        }
      ...
      if ( *(&thisa->field_F41F + i) )
      {
        if ( v14[i] == -1 )
        {
          ...
          if ( byte_4F4C30[thisa->heroType[i]][thisa->heroIconIdxRelated[i] + 1] <= thisa->countRelatedToSpellAnimation[i] )
          {
            ...
            thisa->field_33BD[i] = KBTickCount();
          }
        }
        else
        {
    ...
    combatManager::DrawFrame(thisa, 1, 1, 0, 0, 75, 1, 1);
  }
