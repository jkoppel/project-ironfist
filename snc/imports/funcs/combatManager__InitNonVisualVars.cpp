{
  ...
  for ( i = 0; i < 2; ++i )
  {
    ...
    if ( thisa->heroes[i] )
      thisa->heroSpellpowers[i] = hero::Stats(thisa->heroes[i], PRIMARY_SKILL_SPELLPOWER);
  ...
  combatManager::SetupAdjacencyArray(thisa);
  combatManager::GenerateMap(thisa);
  combatManager::LoadArmies(thisa);
}