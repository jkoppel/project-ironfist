...
void __fastcall SetupHeroView() {
	...
	for(a2 = 0; (signed int)a2 < 4; ++a2) {
		stat = gpHVHero->Stats(a2);
		sprintf(gText, "%d", stat);
		evt.yCoordOrFieldID = a2 + 76;
		evt.payload = gText;
		heroWin->BroadcastMessage(evt);
	}
	twn = gpHVHero->GetOccupiedTown();
	v11 = gpGame->GetLuck(gpHVHero, 0, twn);
	v12 = abs(v11);
	if(v12 <= 0)
		v12 = 1;
	...
	twn2 = gpHVHero->GetOccupiedTown();
	v13 = gpHVHero->army.GetMorale(gpHVHero, twn2, 0);
	v12 = abs(v13);
	if(v12 <= 0)
		v12 = 1;
	for(a2 = 0; (signed int)a2 < 3; ++a2) {
	...
	evt.payload = (void *)4;
	heroWin->BroadcastMessage(evt);
	stat2 = gpHVHero->Stats(PRIMARY_SKILL_KNOWLEDGE);
	sprintf(gText, "%d/%d", gpHVHero->spellpoints, 10 * stat2);
	evt.xCoordOrKeycode = 3;
	...
	for(a2 = 0; (signed int)a2 < 8; ++a2) {
		if(gpHVHero->numSecSkillsKnown <= a2) {
			...
			int skill = gpHVHero->GetNthSS(a2);
			evt.yCoordOrFieldID = a2 + 400;
			evt.xCoordOrKeycode = 4;
			...
			if(skill == SECONDARY_SKILL_WISDOM & gpHVHero->factionID == FACTION_CYBORG)
				evt.payload = str; // Cybernetics name
			else
			...
			evt.xCoordOrKeycode = 3;
			evt.yCoordOrFieldID = a2 + 416;
			skillLevel = gpHVHero->GetSSLevel(skill);
			v5 = skillLevel - gpHVHero->secondarySkillLevel[skill];
			if(v5 <= 0)
