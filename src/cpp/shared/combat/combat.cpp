  ...
  if(!this->field_F2B3) {
      ...
      for(int i = 0; i < 8; ++i) {
        if(v18[i])
          if((CreatureHasAttribute(thisb->creatureIdx, CHARGER) && thisb->GetStraightLineDistanceToHex(a2[i]) <= thisb->creature.speed && thisb->TargetOnStraightLine(a2[i]) && thisb->TargetOnStraightLine(hexIdx) && thisb->ValidFlightPath(a2[i], 0))) {
            ...
            v10[i] = a2[i] == thisb->occupiedHex || thisb->ValidPath(a2[i], 1);
        else
          v10[i] = 0;
      }
    }
    *(DWORD *)&this->_15[64] = 0;
    ...
    for(int i = 0; i < 8; ++i) {
      ...
      if(v10[v17]) {
        if(v15->creature.creature_flags & TWO_HEXER) {
          if(i || this->combatGrid[hexIdx - 1].unitOwner != v4 || this->combatGrid[hexIdx - 1].stackIdx != v13) {
            if(i != 5
              || this->combatGrid[hexIdx + 1].unitOwner != v4
              || this->combatGrid[hexIdx + 1].stackIdx != v13) {
              if(i != 2
                || this->combatGrid[hexIdx - 1].unitOwner != v4
                || this->combatGrid[hexIdx - 1].stackIdx != v13) {
                if(i == 3
