signed int __thiscall CreatureStack::othCreatureIsAdjacent(CreatureStack *this, int side, int idx)
{
  ...
  thisa = this;
  hex = combatManager->creatures[side][idx].occupiedHex;
  if ( combatManager->creatures[side][idx].creature.creature_flags & TWO_HEXER )
    secondHex = hex + (combatManager->creatures[side][idx].owningSide < 1u ? 1 : -1);
  else
    secondHex = -1;
  for ( hexIdx = 0; hexIdx < 6; ++hexIdx )
  {
    neighbor = CreatureStack::getHexNeighbor(thisa, thisa->occupiedHex, hexIdx);
  ...
  if ( thisa->creature.creature_flags & TWO_HEXER )
  {
    ...
    {
      neighbora = CreatureStack::getHexNeighbor(thisa, knownHex, hexIdxa);
