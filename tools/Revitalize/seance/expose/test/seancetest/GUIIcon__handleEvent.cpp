signed int __thiscall GUIIcon::handleEvent(GUIIcon *this, InputEvent *evt)
{
  GUIIcon *thisa; // esi@1
  __int16 v3; // cx@1
  signed int result; // eax@4
  INPUT_EVENT_CODE evtCode; // edx@6
  GUIWindow *parent; // ebp@14
  __int16 xRelParent; // ax@14
  __int16 yRelParent; // bx@14
  AbstractResource *v9; // ST00_4@36
  ...
  if ( !(v3 & 2) )
  {
    ...
    if ( evt->xCoordOrKeycode != 60 )
      return AbstractGUIComponent::handleEvent((AbstractGUIComponent *)thisa, (Event *)evt);
  }
  ...
  switch ( evt->xCoordOrKeycode )
  {
    ...
    case GUI_MESSAGE_SET_ICON:
      ...
      if ( thisa->icon )
      {
        ...
        thisa->icon = ResourceManager::getIconByFilename(resourceManager, (const char *)evt->payload);
      }
      result = 1;
      break;
    case GUI_MESSAGE_REPLACE_ICON:
      ...
      {
        v9 = (AbstractResource *)thisa->icon;
        thisa->iconFileID = (int)evt->payload;
        ResourceManager::reduceReferenceCountToResource(resourceManager, v9);
        thisa->icon = ResourceManager::getIconByFileID(resourceManager, (int)evt->payload);
      }
      result = 0;
      break;
  }
