signed __int64 __thiscall CombatManager::doFlavorAnimations(CombatManager *this)
{
  double v1; // st7@75
  int v2; // ST34_4@85
    ...
    for ( i = 0; combatManager->numCreatures[j] > i; ++i )
    {
      ...
      if ( !(combatManager->creatures[j][i].creature.creature_flags & DEAD)
        ...
        && (v13->animationType >= 8 && v13->animationType <= 12
         || v13->animationType == 7 && v13->baseFidgetTime + v13->form.maxFidgetDelay < getTickCount()) )
      {
        ++v12;
      ...
      for ( i = 0; combatManager->numCreatures[k] > i; ++i )
      {
        ...
        if ( creatureShouldDoSomeAnimation[k][i] )
        {
          if ( v13->animationType == ANIMATION_TYPE_STANDING )
          {
            ...
            v6 = 0.0;
            v13->field_4A = v13->form.field_75 - 1;
            for ( l = 0; v13->form.field_75 - 1 > l; ++l )
            {
              v1 = v13->form.field_76[l] + v6;
              v6 = v1;
              if ( v1 > v5 )
              {
                v13->field_4A = l;
          ...
          {
            ++v13->animationFrame;
            if ( !v13->form.maxFidgetDelay && v13->form.field_75 == 1 && unseededNextRand(0, 100) < 8 )
              --v13->animationFrame;
            if ( v13->form.animationLengths[v13->field_4A + 8] <= v13->animationFrame )
            {
              v13->animationType = 7;
              v13->animationFrame = 0;
              v13->baseFidgetTime = getTickCount();
              if ( v13->form.maxFidgetDelay > 0 )
              {
                v2 = unseededNextRand(0, v13->form.maxFidgetDelay);
                v13->baseFidgetTime = (signed __int64)((double)v2 * 0.5
                                                     - (double)v13->form.maxFidgetDelay * 0.25
