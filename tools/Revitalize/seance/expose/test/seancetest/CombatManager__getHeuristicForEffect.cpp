int __thiscall CombatManager::getHeuristicForEffect(CombatManager *this, CreatureStack *stack, STACK_MODIFYING_EFFECT eff)
{
  __int16 v3; // fps@1
  ...
  thisa = this;
  UNDEF(v3);
  chance = CreatureStack::getSpellSuccessChance(stack, (Spell)spellForEffect[eff]);
  if ( v4 | v5 )
  {
    result = 0;
  }
...
LABEL_7:
          ...
          else
          {
            ...
            {
              if ( CreatureStack::getFreeNeighborBitmask(stack, stack->occupiedHex, 1, -1) == 255 )
              {
...
LABEL_67:
      ...
      case EFFECT_DRAGON_SLAYER:
        ...
        for ( i = 0; thisa->numCreatures[1 - stack->owningSide] > i; ++i )
        {
          ...
          {
            ++numDragons;
            if ( CreatureStack::othCreatureIsAdjacent(
                   stack,
