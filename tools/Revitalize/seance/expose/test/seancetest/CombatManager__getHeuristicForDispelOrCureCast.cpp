void __thiscall CombatManager::getHeuristicForDispelOrCureCast(CombatManager *this, int *val, int side, int hex, int isCure)
{
  double v5; // st7@27
  int v6; // edx@27
  double v7; // st7@30
  ...
  STACK_MODIFYING_EFFECT eff; // [sp+48h] [bp-20h]@22
  CreatureStack *stack; // [sp+4Ch] [bp-1Ch]@13
  ...
  signed int lostTurn; // [sp+64h] [bp-4h]@18
    ...
    for ( i = 0; i < 20; ++i )
    {
        ...
        && thisa->creatures[curSide][i].quantity > 0 )
      {
        ...
        if ( isCure == 1 )
        {
          ...
          removingPosEffectVal = (signed __int64)((double)creature_table[thisa->creatures[curSide][i].creatureIdx].fight_value
                                                * (double)dam
                                                * 0.75
                                                / (double)thisa->creatures[curSide][i].creature.hp
                                                + (double)removingPosEffectVal);
        }
        if ( thisa->creatures[curSide][i].creature.creature_flags & MAYBE_NOT_LOST_TURN )
          lostTurn = 1;
        else
          lostTurn = 0;
        if ( HIBYTE(thisa->creatures[curSide][i].creature.creature_flags) & ATTR_MIRROR_IMAGE )
        {
          removingNegEffectVal = thisa->creatures[curSide][i].quantity
                               * creature_table[thisa->creatures[curSide][i].creatureIdx].fight_value;
        }
        else
        {
            ...
            if ( stack->effectStrengths[eff] )
            {
              switch ( eff )
              {
                ...
                case EFFECT_STEELSKIN:
                  v5 = (double)CombatManager::getHeuristicForEffect(thisa, stack, eff);
                ...
                case EFFECT_SLOW:
                case EFFECT_CURSE:
                  v7 = (double)CombatManager::getHeuristicForEffect(thisa, stack, eff);
                ...
                case EFFECT_PETRIFY:
                  v9 = (double)CombatManager::getHeuristicForEffect(thisa, stack, eff);
