void __thiscall AdvManager::drawAdventureMapTile(AdvManager *this, int x, int y, int a3, int a4, char a5, int a6)
{
  ...
  dword_524D10 = 32 * a3;
  ::y = 32 * a4;
  currentlyDrawnMapTile = AdvManager::getTile(this, x, y);
  if ( !dword_4F7470 && ((signed __int64)__PAIR__(y, x) < 0 || x >= mapWidth || y >= mapHeight) )
  {
    idx = -1;
    if ( x == -1 )
    {
    ...
    drawTile(this->stonTileset, idx, HeroWindowManager::instance->screenBuffer, dword_524D10, ::y);
    return;
  }
  if ( !dword_4F7470 && (unsigned __int8)(*(&tileExplored[x] + mapWidth * y) & byte_5306F0) || dword_4F5460 )
  {
    dword_524CF8 = 0;
  }
  else
  ...
  if ( !(a5 & 0x20) || dword_4F5460 )
  {
    if ( a5 & 1 )
    {
      ...
      {
        if ( !dword_4F5460 || byte_4F0B50[(LOBYTE(currentlyDrawnMapTile->field_2) >> 2) & 0x3F] )
        {
          rasterize(
      ...
      if ( (currentlyDrawnMapTile->flags >> 1) & 1
        ...
        && (!dword_4F5460 || byte_4F0B50[(LOBYTE(currentlyDrawnMapTile->field_2) >> 2) & 0x3F]) )
      {
        rasterize(
          (Icon *)this->field_CE[(LOBYTE(currentlyDrawnMapTile->field_2) >> 2) & 0x3F],
          HeroWindowManager::instance->screenBuffer,
    ...
    if ( (a5 & 8 || a5 & 0x80) && !dword_4F5460 )
    {
      ...
      if ( a5 & 8 )
      {
        if ( x > 0 )
        {
          dword_524D08 = (int)AdvManager::getTile(this, x - 1, y);
        ...
        if ( ((LOBYTE(currentlyDrawnMapTile->field_2) >> 2) & 0x3F) == 12 )
        {
          if ( this->field_2B2 != x || this->field_2B6 != y )
          {
            ...
            if ( HIBYTE(currentlyDrawnMapTile->field_2) != 59 && HIBYTE(currentlyDrawnMapTile->field_2) != 60 )
              dword_524D14 = (unsigned __int8)byte_50F100[*(&this->field_20A + (x & 3))];
            else
      ...
      else
      {
        ...
        {
          dword_524CDC = (int)&gameObject->heroes[(unsigned __int8)((unsigned __int8)(currentlyDrawnMapTile->flags >> 8) >> -5)];
    ...
    if ( a5 & 4 || a5 & 0x40 )
    {
      ...
      {
        if ( !dword_4F5460 || byte_4F0B50[((unsigned __int8)currentlyDrawnMapTile->field_6 >> 2) & 0x3F] )
        {
          rasterize(
  ...
  else
  {
    ...
    {
      if ( this->field_A2 && *(_WORD *)(2 * x + 2 * mapWidth * y + this->sizeOfSomethingMapRelated) )
      {
        if ( (*(_WORD *)(2 * x + 2 * mapWidth * y + this->sizeOfSomethingMapRelated) >> 8) & 1 )
          rasterizeWithSubstitution(
            (Icon *)this->field_CE[17],
            HeroWindowManager::instance->screenBuffer,
            dword_524D10 - 12,
            ::y + 2,
            (unsigned __int8)(*(_WORD *)(2 * x + 2 * mapWidth * y + this->sizeOfSomethingMapRelated) - 1),
            1,
            0,
            0,
            0x1E0u,
