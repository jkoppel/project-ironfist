void __thiscall sub_4C3DD0(CombatManager *this, CreatureStack *stack)
{
  int v2; // ST24_4@3
  int v3; // ST28_4@3
  ...
  thisa = this;
  if ( !combatGraphicsOff && combatShadeLevel >= 1 )
  {
    v5 = CreatureStack::getFreeNeighborBitmask(stack, stack->occupiedHex, 2, -1);
    ...
    for ( i = 0; i < 117; ++i )
    {
      ...
      {
        if ( thisa->combatGrid[i].field_41 )
        {
          ...
          {
            if ( thisa->combatGrid[i].unitOwner != stack->owningSide )
              thisa->field_49F[i] = 1;
          }
        }
        else
