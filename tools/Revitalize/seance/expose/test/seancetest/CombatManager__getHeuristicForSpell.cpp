void __thiscall CombatManager::getHeuristicForSpell(CombatManager *this, Spell spell, int *value, int *target)
{
  int durationBenefitIdx; // eax@30
  int v5; // ST94_4@56
  double v6; // st7@57
  int v7; // ecx@57
  int v8; // ST88_4@64
  ...
  signed int v32; // [sp+A0h] [bp-40h]@154
  int v33; // [sp+A4h] [bp-3Ch]@105
  int duration; // [sp+A8h] [bp-38h]@26
  ...
  CreatureStack *potTarg; // [sp+C4h] [bp-1Ch]@1
  AI_SPELL_TYPES aiSpellType; // [sp+C8h] [bp-18h]@2
  int i; // [sp+D0h] [bp-10h]@154
  int targetHex; // [sp+D4h] [bp-Ch]@1
  int targSpentTurn; // [sp+D8h] [bp-8h]@1
...
LABEL_12:
      if ( aiSpellType == AI_RESURRECT_SPELL )
        targetHex = CombatManager::findHexWithCorpse(this, 1, &hex, spell);
      if ( aiSpellType == AI_TARGET_BUFF_SPELL
        || aiSpellType == AI_TARGET_DAMAGE_OR_DEBUFF_SPELL
        || aiSpellType == AI_MASS_DEBUFF_SPELL
        || aiSpellType == AI_MASS_BUFF_SPELL
        || aiSpellType == AI_DISPEL_SPELL )
        targetHex = CombatManager::findCreature(thisa, 1, targetSide, &hex);
      while ( 2 )
      {
        if ( !targetHex )
        {
          ...
          else
          {
            ...
            dword_526C20 = thisa->combatGrid[hex].unitOwner;
            if ( potTarg->creature.creature_flags & MAYBE_NOT_LOST_TURN )
              targSpentTurn = 1;
            else
              targSpentTurn = 0;
          ...
          {
            case SPELL_CURE:
              CombatManager::getHeuristicForDispelOrCureCast(thisa, &val, thisa->currentActionSide, hex, 1);
              goto LABEL_174;
            case SPELL_MASS_CURE:
              CombatManager::getHeuristicForDispelOrCureCast(thisa, &val, thisa->currentActionSide, -1, 1);
              goto LABEL_174;
            case SPELL_DISPEL_MAGIC:
              CombatManager::getHeuristicForDispelOrCureCast(thisa, &val, potTarg->owningSide, potTarg->stackIdx, 0);
              goto LABEL_174;
            case SPELL_MASS_DISPEL:
              CombatManager::getHeuristicForDispelOrCureCast(thisa, &val, 2, -1, 0);
              goto LABEL_174;
            case SPELL_RESURRECT:
            case SPELL_RESURRECT_TRUE:
            case SPELL_ANIMATE_DEAD:
              if ( !underMyControl )
                CombatManager::getHeuristicForRessurect(thisa, &val, hex, spell);
              goto LABEL_174;
            case SPELL_MIRROR_IMAGE:
            case SPELL_SUMMON_EARTH_ELEMENTAL:
            case SPELL_SUMMON_AIR_ELEMENTAL:
            case SPELL_SUMMON_FIRE_ELEMENTAL:
            case SPELL_SUMMON_WATER_ELEMENTAL:
              val = CombatManager::getHeuristicForSummon(thisa, hex, spell);
              goto LABEL_174;
            case SPELL_FIREBALL:
            case SPELL_FIREBLAST:
            case SPELL_LIGHTNING_BOLT:
            ...
            case SPELL_DEATH_WAVE:
              if ( !underMyControl )
              {
                CombatManager::getHeuristicForDamageSpell(thisa, &val, spell, hex);
                if ( incapacitated )
            ...
            case SPELL_MASS_HASTE:
              if ( !incapacitated )
              {
                if ( !underMyControl )
                {
                  ...
                  {
                    v5 = CombatManager::getHeuristicForEffect(thisa, potTarg, 0);
                    val = (signed __int64)((double)v5 * durBenefit);
                    if ( potTarg->effectStrengths[1] )
                    {
                      v6 = (double)CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_SLOW);
            ...
            case SPELL_BERZERKER:
              if ( !incapacitated )
              {
                if ( !underMyControl )
                {
                  ...
                  {
                    v8 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_BERSERKER);
                    val = (signed __int64)((double)v8 * durBenefit);
                    if ( potTarg->effectStrengths[7] )
                    {
                      v9 = (double)CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_HYPNOTIZE);
            ...
            case SPELL_HYPNOTIZE:
              if ( !incapacitated )
              {
                if ( !underMyControl )
                {
                  if ( !potTarg->effectStrengths[7] )
                  {
                    v11 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_HYPNOTIZE);
                    val = (signed __int64)((double)v11 * durBenefit);
                    if ( potTarg->effectStrengths[5] )
                    {
                      v12 = (double)CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_BERSERKER);
            ...
            case SPELL_MASS_SLOW:
              if ( !incapacitated )
              {
                if ( !underMyControl )
                {
                  if ( !potTarg->effectStrengths[1] )
                  {
                    v14 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_SLOW);
                    val = (signed __int64)((double)v14 * durBenefit);
                    if ( potTarg->effectStrengths[0] )
                    {
                      v15 = (double)CombatManager::getHeuristicForEffect(thisa, potTarg, 0);
            ...
            case SPELL_MASS_BLESS:
              if ( !incapacitated )
              {
                if ( !underMyControl )
                {
                  ...
                  {
                    v17 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_BLESS);
                    val = (signed __int64)((double)v17 * durBenefit);
                    if ( potTarg->effectStrengths[4] )
                    {
                      v18 = (double)CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_CURSE);
            ...
            case SPELL_MASS_CURSE:
              if ( !incapacitated )
              {
                if ( !underMyControl )
                {
                  if ( !potTarg->effectStrengths[4] )
                  {
                    v20 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_CURSE);
                    val = (signed __int64)((double)-v20 * durBenefit);
                    if ( potTarg->effectStrengths[3] )
                    {
                      v21 = (double)CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_BLESS);
            ...
            case SPELL_ANTI_MAGIC:
              if ( !potTarg->effectStrengths[12] )
              {
                ...
                {
                  v23 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_ANTI_MAGIC);
                  val = (signed __int64)((double)v23 * durBenefit);
                  CombatManager::getHeuristicForDispelOrCureCast(thisa, &v33, potTarg->owningSide, potTarg->stackIdx, 0);
                  val += v33;
                }
                else
                {
            ...
            case SPELL_STONESKIN:
              ...
              {
                v24 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_STONESKIN);
            ...
            case SPELL_STEELSKIN:
              if ( !underMyControl && !potTarg->effectStrengths[13] && !potTarg->effectStrengths[14] )
              {
                v25 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_STEELSKIN);
            ...
            case SPELL_BLOOD_LUST:
              ...
              {
                v26 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_BLOOD_LUST);
            ...
            case SPELL_MASS_SHIELD:
              ...
              {
                v27 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_SHIELD);
            ...
            case SPELL_BLIND:
              ...
              {
                v28 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_BLIND);
            ...
            case SPELL_PARALYZE:
              ...
              {
                v29 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_PARALYZE);
            ...
            case SPELL_DRAGON_SLAYER:
              ...
              {
                v30 = CombatManager::getHeuristicForEffect(thisa, potTarg, EFFECT_DRAGON_SLAYER);
...
LABEL_174:
              ...
              switch ( aiSpellType )
              {
                ...
                case AI_TARGET_DAMAGE_OR_DEBUFF_SPELL:
                case AI_DISPEL_SPELL:
                  targetHex = CombatManager::findCreature(thisa, hex + 1, targetSide, &hex);
                  break;
                case AI_RESURRECT_SPELL:
                  targetHex = CombatManager::findHexWithCorpse(thisa, hex + 1, &hex, spell);
                  break;
                case AI_BALL_SPALL:
                  nextRealHexIdx(&hex);
                  if ( hex > 43 )
