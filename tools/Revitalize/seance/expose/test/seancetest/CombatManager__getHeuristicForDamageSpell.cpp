int __thiscall CombatManager::getHeuristicForDamageSpell(CombatManager *this, int *val, Spell spell, int hexIdx)
{
  __int16 v4; // fps@76
  unsigned __int8 v5; // c0@76
  unsigned __int8 v6; // c3@76
  int unitOwner; // eax@113
  int v8; // ecx@114
  __int64 v9; // qax@115
  __int64 v10; // qax@115
  signed __int64 v11; // qax@128
  CombatManager *thisa; // [sp+50h] [bp-54h]@1
  int v14; // [sp+54h] [bp-50h]@117
  CREATURES creatIdx; // [sp+58h] [bp-4Ch]@77
  int remDam; // [sp+5Ch] [bp-48h]@110
  int stacksKilled[2]; // [sp+60h] [bp-44h]@22
  float chance; // [sp+68h] [bp-3Ch]@76
  int v19; // [sp+6Ch] [bp-38h]@18
  int damage; // [sp+70h] [bp-34h]@77
  CreatureStack *stack; // [sp+74h] [bp-30h]@19
...
LABEL_32:
        if ( (neighborIdx < 7 || spell == SPELL_FIREBLAST) && neighborIdx < 19 )
        {
          ...
          if ( neighborIdx > 0 && neighborIdx <= 6 )
            curHex = getHexNeighbor(hexIdx, neighborIdx - 1);
          if ( neighborIdx > 6 && neighborIdx <= 12 )
          {
            curHex = getHexNeighbor(hexIdx, neighborIdx - 7);
            curHex = getHexNeighbor(curHex, neighborIdx - 7);
          }
          if ( neighborIdx == 13 )
            curHex = hexIdx - 26;
          if ( neighborIdx == 14 )
          ...
          if ( neighborIdx == 15 )
          {
            curHex = getHexNeighbor(hexIdx, 1);
            curHex = getHexNeighbor(curHex, 0);
          }
          if ( neighborIdx == 16 )
          {
            curHex = getHexNeighbor(hexIdx, 1);
            curHex = getHexNeighbor(curHex, 2);
          }
          if ( neighborIdx == 17 )
          {
            curHex = getHexNeighbor(hexIdx, 4);
            curHex = getHexNeighbor(curHex, 5);
          }
          if ( neighborIdx == 18 )
          {
            curHex = getHexNeighbor(hexIdx, 4);
            curHex = getHexNeighbor(curHex, 3);
          }
          ++neighborIdx;
        }
        else
      ...
      case SPELL_CHAIN_LIGHTNING:
        ...
        {
          dam >>= 1;
          curHex = CombatManager::findClosestCreature(
                     thisa,
        ...
        if ( curHex < 116 )
        {
          if ( thisa->combatGrid[curHex].stackIdx >= 0 )
          {
            if ( thisa->combatGrid[curHex].unitOwner >= 0 )
            {
              ...
              if ( !stack_affected_by_aoe[thisa->combatGrid[curHex].unitOwner][thisa->combatGrid[curHex].stackIdx] )
              {
                stack_affected_by_aoe[thisa->combatGrid[curHex].unitOwner][thisa->combatGrid[curHex].stackIdx] = 1;
                UNDEF(v4);
                chance = CreatureStack::getSpellSuccessChance(stack, spell);
                if ( !(v5 | v6) )
                {
  ...
  {
    if ( v30[thisa->currentActionSide] > 0 )
      *val = v19 + v30[1 - thisa->currentActionSide] - v30[thisa->currentActionSide];
    else
      *val = v19 + 100000000 - *(_WORD *)&spell_table[spell].appearingChance;
  }
  LODWORD(v11) = thisa;
