void __thiscall CombatManager::initializeCreatureStacks(CombatManager *this)
{
  ...
  for ( j = 0; j < 2; ++j )
  {
    for ( armyIdxa = 0; armyIdxa < 20; ++armyIdxa )
      CreatureStack::initializeBookkeeping(&thisa->creatures[j][armyIdxa]);
  }
  for ( armyIdxb = 0; armyIdxb < 5; ++armyIdxb )
  {
    if ( thisa->armies[0]->creatureTypes[armyIdxb] != -1 )
    {
      ...
      CreatureStack::init(
        (CreatureStack *)thisa->creatures + thisa->numCreatures[0],
        ...
        attackerOccHex,
        armyIdxb);
      CreatureStack::loadRelatedFiles((CreatureStack *)thisa->creatures + thisa->numCreatures[0]++);
    }
    if ( thisa->armies[1]->creatureTypes[armyIdxb] != -1 )
    {
      ...
      CreatureStack::init(
        ...
        defenderOccHex,
        armyIdxb);
      CreatureStack::loadRelatedFiles(&thisa->creatures[1][thisa->numCreatures[1]++]);
    }
  }
}
