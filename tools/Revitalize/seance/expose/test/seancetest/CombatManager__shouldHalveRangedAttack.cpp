signed int __thiscall CombatManager::shouldHalveRangedAttack(CombatManager *this, int side, signed int occupiedHex, signed int targHex)
{
  ...
  if ( this->isCastleBattle )
  {
    ...
    {
      if ( !isBehindCastle(occupiedHex) && isBehindCastle(targHex) )
      {
        ...
        {
          v15 = abs(rowDiff);
          v16 = (double)((((rowDiff <= 0) - 1) & 2) - 1);
          v18 = (double)colDiff / (double)abs(rowDiff);
        }
        else
        {
          v15 = abs(colDiff);
          v18 = (double)((((colDiff <= 0) - 1) & 2) - 1);
