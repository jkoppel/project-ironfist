int __stdcall sub_4B27A0(int a1)
{
  int result; // eax@4
  ...
  for ( i = 0; i < 7; ++i )
  {
    v7[i] = (signed __int64)(((double)(5 * *(_DWORD *)&v2[4 * i + 52]) * 0.7
                            + (double)gameObject->players[a1].resources[i])
  ...
  for ( i = 0; i < 7; ++i )
  {
    v3[i] = (double)v7[i] / v4;
    result = i;
    flt_530718[i] = (double)dword_4F1DE0[i] / (v3[i] / 2.0 + 0.5);
  }
  return result;
}
