int __thiscall GameInfo::getRandomHeroFromHeroPool(GameInfo *this, int playerIdx, signed int faction, int getPowerfulHero)
{
  GameInfo *thisa; // [sp+Ch] [bp-18h]@1
    ...
    if ( thisa->relatedToHeroForHireStatus[heroIdx] == -1 || thisa->relatedToHeroForHireStatus[heroIdx] == 64 )
    {
         ...
         || thisa->heroes[heroIdx].artifacts[1] != -1 && thisa->heroes[heroIdx].artifacts[1] != ARTIFACT_MAGIC_BOOK) )
      {
        if ( !loadOrNewGameSelected
          || tries >= 500
          || thisa->heroes[heroIdx].heroID < 54
          || thisa->heroes[heroIdx].heroID > 59 )
          break;
      }
    }
  }
