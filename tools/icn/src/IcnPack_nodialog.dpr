...
// Heroes 1 doesn't indicate cadres with masks in any way, so...
...
begin
  ...
  try
    ...
    begin
      ...
      b.LoadFromFile(outPath + files[i]);
...
{$IFDEF Unpack}
         ...
         'Extracts 1 cadre of each ICN file into OutFolder.'#13#10 +
         'OutFolder defaults to "InFolder\Icn Preview".'#13#10#13#10 +
{$ELSE}
         'Usage 2:  ' + AppName + '.exe Target.icn.cadres'#13#10 +
         'Creates "Target.icn" using cadres from "Target.icn.cadres" folder.'#13#10#13#10 +
{$ENDIF}
